	    ...
	    if(connectDB != null) {
	    	if(Service.sanitizeString(request.getParameter("email").getBytes(), "email")) {
				...
				if(Service.logInSystem(connectDB, request, response, resource) != null){
			    	...
			    	Cookie userCookie = new Cookie("loggedUser", loggedUser.getIdUtente());
			    	userCookie.setHttpOnly(true);	//Rende il cookie inaccessibile da Document script per evitare attacchi Cross Site Scripting XSS
			    	userCookie.setMaxAge(60*Integer.parseInt(resource.getString("lifeTimeCookie")));
